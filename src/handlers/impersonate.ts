...
export async function handleImpersonate(message: Message, client: Client): Promise<void> {
  ...
  if (!playerName || rest.length === 0) {
    await message.reply(t(getLang(message.author.id), 'impersonateUsage'));
    return;
  }
  ...
  if (!fakePlayer) {
    await message.reply(t(getLang(message.author.id), 'impersonateUnknownPlayer', { name: playerName }));
    return;
  }
  ...
  if (!entry) {
    ...
    await message.reply(`❌ \`!as\` does not support \`${subCmd}\`. Supported: \`${supported}\`.`);
    return;
  }
  ...
  const fakeI = new FakeDayInteraction(fakePlayer.userId, channelId, message, argInput);
