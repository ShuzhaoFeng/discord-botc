...
import { getLang, getRoleGuide, t } from '../i18n';
...
function buildList(lang: Lang): string {
  const sections = CATEGORY_ORDER.map((cat) => {
    ...
    const names = roles.map((r) => (lang === 'zh' ? r.nameZh : r.name)).join(' · ');
...
function buildDetail(lang: Lang, role: Role): string {
  const name = lang === 'zh' ? role.nameZh : role.name;
  const cat = catLabel(lang, role.category);
  const ability = lang === 'zh' ? role.descriptionZh : role.description;
...
export async function handleRulebook(
  ...
  if (!role) {
    await interaction.reply({
      content: t(lang, 'rulebookRoleNotFound', roleInput),
      ephemeral: true,
    });
    return;
  }

  await interaction.reply({ content: buildDetail(lang, role), ephemeral: true });
}
